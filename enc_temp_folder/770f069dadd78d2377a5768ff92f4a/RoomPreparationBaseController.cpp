...
void ARoomPreparationBaseController::BeginPlay() {
	...
	for (AActor* Actor : FoundRobots) {
		...
		if (Cleaner != nullptr) {
			...
			CleanerRobot->OnTaskFinished.AddDynamic(this, &ARoomPreparationBaseController::CurrentTaskFinished);
		}
	}
...
void ARoomPreparationBaseController::ExecuteClean(FString RobotName, ARoomPreparation* Room) {
	NumberOfTask = 1;
	CleanerRobot->StartNewTask(ESkillEnum::CLEAN_ROOM, Room);
}

void ARoomPreparationBaseController::ExecuteOpenDoor(FString RobotName, ARoomPreparation* Room) {
	NumberOfTask = 1;
	CleanerRobot->StartNewTask(ESkillEnum::OPEN_DOOR, Room);
}

void ARoomPreparationBaseController::ExecuteSanitizeRobot(FString RobotName, ARoomPreparation* Room) {
	NumberOfTask = 1;
