...
UCLASS()
class ROBOTBT_API UTempoSceneCaptureComponent2D : public USceneCaptureComponent2D, public ITempoSensorInterface
{
	GENERATED_BODY()
...
protected:
	...
	// Armazena requisições pendentes
	std::atomic<bool> bHasRequest = false;

	// Derived components must override this to create new texture reads, based on their current settings, to be enqueued.
	virtual FTextureRead* MakeTextureRead() { return nullptr; };
