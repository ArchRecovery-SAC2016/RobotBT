...
ARoomPreparation* AExperiment::GetRoomByName(FString DoorName) {
	for (auto Room : Rooms) {
...
void AExperiment::ExecuteClean(FString RobotName, ARoomPreparation* Room) {
	NumberOfTask = 1;
	CleanerRobot->StartNewTask(ESkillEnum::CLEAN_ROOM, Room);
}

void AExperiment::ExecuteOpenDoor(FString RobotName, ARoomPreparation* Room) {
	NumberOfTask = 1;
	CleanerRobot->StartNewTask(ESkillEnum::OPEN_DOOR, Room);
}

void AExperiment::ExecuteSanitizeRobot(FString RobotName, ARoomPreparation* Room) {
	NumberOfTask = 1;
	CleanerRobot->StartNewTask(ESkillEnum::SANITIZE_ROBOT, Room);
}

bool AExperiment::EvaluatePreCondition(const FPredicate& Predicate) {
...
void AExperiment::ExecuteMoveFurniture(FString RobotName, ARoomPreparation* Room) {
	NumberOfTask = 2;
...
FValidationStruct AExperiment::GetValidationStruct() {
...
void AExperiment::ValidateExperiment(FValidationStruct ValidationStruct) {
	FString RequestBody;
...
void AExperiment::HandleValidationSuccess(const FString& ResponseContent) {
...
void AExperiment::HandleValidationFailure() {
